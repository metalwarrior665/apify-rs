...
impl Actor {
    ...
    pub async fn openDataset(&mut self, datasetNameOrId: Option<&str>, force_cloud: bool) -> DatasetHandle {
        ...
        if let Some(dataset) = self.dataset_cache.remove(datasetNameOrId.unwrap_or("default")) {
            self.dataset_cache.insert(dataset.id.clone(), dataset.clone());
            return dataset;
        }

        let is_default = datasetNameOrId.is_none();
        ...
        if is_on_apify() || force_cloud {
            if is_default {
                ...
                let cloud_dataset = self.client.create_dataset(datasetNameOrId.unwrap()).send().await.unwrap();
                dataset = DatasetHandle {
                    id: cloud_dataset.id,
            ...
            let name = datasetNameOrId.unwrap_or("default");
    ...
    -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let  dataset_handle = self.openDataset(None, false).await;
